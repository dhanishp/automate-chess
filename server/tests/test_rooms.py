...
from app.game.rooms import RoomAccessError, RoomJoinError, RoomService
...
def test_room_creation_assigns_white_and_waiting_status() -> None:
    ...
    assert response.room.white_player.player_token == response.player_token
...
def test_black_leave_returns_room_to_waiting_without_corruption() -> None:
    service = RoomService(engine_provider=FakeEngineProvider())
    created = service.create_room(CreateRoomRequest())
    joined = service.join_room(JoinRoomRequest(room_code=created.room.room_code))

    room, side = service.leave_room(created.room.room_code, LeaveRoomRequest(player_token=joined.player_token))
